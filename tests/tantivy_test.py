...
class TestClass(object):
    ...
    def test_faceted_search(self, ram_index_with_facet_field):
        index = ram_index_with_facet_field
        query = index.parse_query("facet:/a")
        searcher = index.searcher()
        result = searcher.search(query, 10, facet_axes=[("facet", [tantivy.Facet.from_string("/a")])])
        assert result.count == 2
        assert len(result.facet_axes) == 1
        assert result.facet_axes[0][0] == "facet"
        facet_counts = {f: c for f, c in result.facet_axes[0][1].get(tantivy.Facet.from_string("/a"))}
        assert facet_counts[tantivy.Facet.from_string("/a/b")] == 1
        assert facet_counts[tantivy.Facet.from_string("/a/c")] == 1
