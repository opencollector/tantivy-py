...
from tantivy import SchemaBuilder, Index, Facet, Document
...
def create_index_with_facet_field(dir=None):
    schema = schema_with_facet_field()
    index = Index(schema, dir)
    writer = index.writer(15_000_000, 1)

    doc = Document()
    doc.add_integer("id", 1)
    doc.add_facet("facet", Facet.from_string("/a/b"))
    writer.add_document(doc)

    doc = Document.from_dict(
        {
            "id": 2,
            "facet": Facet.from_string("/a/c"),
        },
        schema
    )
    writer.add_document(doc)
...
@pytest.fixture(scope="class")
def ram_index_with_facet_field():
    return create_index_with_facet_field()
