    ...
    fn into_py<'py>(self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        Ok(match self {
            tv::query::QueryParserError::SyntaxError(message) => {
                SyntaxError { message }.into_pyobject(py)?.into_any()
            }
            tv::query::QueryParserError::UnsupportedQuery(message) => {
                UnsupportedQueryError { message }.into_pyobject(py)?.into_any()
            }
            tv::query::QueryParserError::FieldDoesNotExist(field) => {
                FieldDoesNotExistError { field }.into_pyobject(py)?.into_any()
            }
            tv::query::QueryParserError::FieldDoesNotHavePositionsIndexed(
                field,
            ) => FieldDoesNotHavePositionsIndexedError { field }.into_pyobject(py)?.into_any(),
            tv::query::QueryParserError::ExpectedInt(parse_int_error) => {
                ExpectedIntError { parse_int_error }.into_pyobject(py)?.into_any()
            }
            tv::query::QueryParserError::ExpectedFloat(parse_float_error) => {
                ExpectedFloatError { parse_float_error }.into_pyobject(py)?.into_any()
            }
            tv::query::QueryParserError::ExpectedBool(parse_bool_error) => {
                ExpectedBoolError { parse_bool_error }.into_pyobject(py)?.into_any()
            }
            tv::query::QueryParserError::ExpectedBase64(decode_error) => {
                ExpectedBase64Error { decode_error }.into_pyobject(py)?.into_any()
            }
            tv::query::QueryParserError::AllButQueryForbidden => {
                AllButQueryForbiddenError.into_pyobject(py)?.into_any()
            }
            tv::query::QueryParserError::NoDefaultFieldDeclared => {
                NoDefaultFieldDeclaredError.into_pyobject(py)?.into_any()
            }
            tv::query::QueryParserError::FieldNotIndexed(field) => {
                FieldNotIndexedError { field }.into_pyobject(py)?.into_any()
            }
            tv::query::QueryParserError::PhrasePrefixRequiresAtLeastTwoTerms {
                phrase,
                tokenizer,
            } => {
                PhrasePrefixRequiresAtLeastTwoTermsError { phrase, tokenizer }.into_pyobject(py)?.into_any()
            }
            tv::query::QueryParserError::UnknownTokenizer { tokenizer, field } => {
                    UnknownTokenizerError { tokenizer, field }.into_pyobject(py)?.into_any()
            }
            tv::query::QueryParserError::RangeMustNotHavePhrase => {
                RangeMustNotHavePhraseError.into_pyobject(py)?.into_any()
            }
            tv::query::QueryParserError::DateFormatError(_) => {
                DateFormatError { inner: self }.into_pyobject(py)?.into_any()
            }
            tv::query::QueryParserError::FacetFormatError(facet_parse_error) => {
                FacetFormatError { facet_parse_error }.into_pyobject(py)?.into_any()
            }
            tv::query::QueryParserError::IpFormatError(addr_parse_error) => {
                IpFormatError { addr_parse_error }.into_pyobject(py)?.into_any()
            }
        })
    }
}
